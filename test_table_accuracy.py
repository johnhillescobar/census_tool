    ...
    for query, expected_table, category in test_cases:
        ...
        try:
            ...
            if results:
                ...
                print(f"Found: {found_table} (confidence: {confidence:.2f}, distance: {distance:.3f}) {status}")
                ...
                if is_correct:
                    ...
                    for i, result in enumerate(results[:5], 1):
                        ...
                        print(f"    {i}. {table_code} (conf: {conf:.2f}, dist: {dist:.3f}) {marker}")
            ...
            for fail in fails:
                ...
                if fail['top_5']:
                    ...
                    if in_top_5:
                        ...
                        print(f"    ⚠️  Expected table IS in top 5 (rank #{rank}) - scoring issue")
                    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test Phase 8 table selection accuracy")
    parser.add_argument("--analyze", type=str, help="Deep dive into a specific query")
    parser.add_argument("--expected", type=str, help="Expected table for --analyze query")
    ...
    if args.analyze and args.expected:
        ...
        if accuracy < 85:
            ...
            print('   python test_table_accuracy.py --analyze "median household income" --expected B19013')
            print()
            print("2. Check if expected tables are in results:")
            print("   - If YES: Scoring issue → adjust calculate_table_confidence_score()")
            print("   - If NO: Retrieval issue → improve table descriptions in build_index_tables.py")
            print()
