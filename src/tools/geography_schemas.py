...
class AreaResolutionInput(BaseModel):
    ...
    geography_type: GeographyLevel = Field(default=GeographyLevel.STATE, description="Geography or summary level to enumerate")
    ...
    parent: Optional[Dict[str, str]] = Field(default=None, description="Parent geography or summary level like {'state': '06'} for example if you want to enumerate all counties in California, you would need to know the state code for California which is 06")
...
class GeographyEnumerationInput(BaseModel):
    ...
    dataset: str = Field(default="acs/acs5", description="A census dataset is a collection of statistical information gathered from every individual or household in a specific region, used for demographic, social, and economic analysis")
    year: int = Field(default=2023, description="Census year which is the year of the data you want to analyze")
    parent: Optional[Dict[str, str]] = Field(default=None, description="Parent geography constraint like {'state': '06'} for example if you want to enumerate all counties in California, you would need to know the state code for California which is 06")
