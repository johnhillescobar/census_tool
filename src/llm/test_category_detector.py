...
def test_distance_boosting():
    ...
    boosted = boost_category_results(results, 'subject', confidence=1.0, boosts_amount=0.05)
    
    print("\nAfter boosting 'subject' category (confidence=1.0, boost=0.05):")
    for id, dist, meta in zip(boosted['ids'][0], boosted['distances'][0], boosted['metadatas'][0]):
        print(f"  {id}: {dist:.3f} ({meta['category']})")
    ...
    assert abs(actual_s0101 - expected_s0101) < 0.001, f"S0101 boost failed: expected {expected_s0101:.3f}, got {actual_s0101:.3f}"
...
def test_reranking():
    ...
    for i, (id, dist) in enumerate(zip(reranked['ids'][0], reranked['distances'][0]), 1):
        print(f"  {i}. {id}: {dist:.3f}")
    
    # S0101 should now be first
    print(f"\nChecking rank #1: {reranked['ids'][0][0]} (expected: S0101)")
    assert reranked['ids'][0][0] == 'S0101', f"Expected S0101 first, got {reranked['ids'][0][0]}"
    print("✅ S0101 is ranked #1")
    
    print(f"Checking distance: {reranked['distances'][0][0]:.3f} (expected: 0.344)")
    assert abs(reranked['distances'][0][0] - 0.344) < 0.001, f"Expected distance 0.344, got {reranked['distances'][0][0]:.3f}"
