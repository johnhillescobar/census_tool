    ...
    def _parse_solution(self, result: Dict) -> Dict:
        ...
        try:
            ...
            if isinstance(parsed, dict) and "census_data" in parsed:
                ...
                logger.debug(f"Parsed JSON but missing census_data key. Keys: {parsed.keys() if isinstance(parsed, dict) else 'not a dict'}")
        ...
        single_line_match = re.search(r'\{[^{}]*"census_data"[^{}]*(?:\{(?:[^{}]*|\{[^{}]*\})*\}[^{}]*)*\}', output)
            ...
            if isinstance(step, dict) and "tool_output" in step:
                ...
                if isinstance(tool_output, str):
                    try:
                        ...
                        if isinstance(parsed_output, dict) and parsed_output.get("success") and "data" in parsed_output:
                            census_result = parsed_output["data"]
                            if isinstance(census_result, dict) and "data" in census_result:
                                ...
                                if len(census_data_rows) > 1:
                                    ...
                                    return {
                                        "census_data": {"data": census_data_rows, "variables": {"B01003_001E": "Total Population"}},
        ...
        logger.warning("Agent did not return valid JSON - agent may have hit iteration limit")
