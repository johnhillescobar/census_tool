...
def test_enumerate_counties():
    ...
    assert len(counties) == 58, f"California should have 58 counties, found {len(counties)}"
    print("\n✅ Test passed!")
...
def test_find_county_code():
    ...
    if result:
        ...
        assert result['code'] == '037', f"LA County code should be 037, got {result['code']}"
        print("\n✅ Test passed!")
    else:
        print("❌ Not found!")
...
def test_enumerate_states():
    ...
    states = registry.enumerate_areas(
        dataset="acs/acs5",
        year=2023,
        geo_token="state"
    )
