...
def generate_session_pdf(
    ...
    story.append(Spacer(1, 60))
    story.append(Paragraph("<i>This report contains all questions, answers, charts, and data tables from your census data session.</i>", meta_style))
                ...
                if "Chart created successfully:" in file_info:
                    ...
                    try:
                        if Path(filepath).exists():
                            ...
                            story.append(Paragraph(f"⚠️ Chart file not found: {Path(filepath).name}", meta_style))
                    ...
                    if not table_embedded:
                        story.append(Paragraph(f"📁 Table saved to: {Path(table_path).name}", meta_style))
...
def _create_pdf_table_from_data(table_data: List[List], title: str) -> Table:
    ...
    try:
        ...
        table.setStyle(TableStyle([
            ...
            # Column width
            ('COLWIDTH', (0, 0), (0, -1), 1.5*inch),  # First column (location names) wider
        ]))
