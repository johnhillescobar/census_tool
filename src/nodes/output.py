    ...
    try:
        ...
        if chart_type == "line":
            ...
            for col in numeric_columns:
                if any(year_indicator in col.upper() for year_indicator in ["YEAR", "TIME", "DATE"]):
                    y_column = col
                    break
            ...
            if not y_column:
                for col in numeric_columns:
                    if len(col) > 8 and col.startswith(("B", "C", "S", "DP")):  # Census variable patterns
                        y_column = col
                        break
        ...
        if not y_column:
            ...
            y_column = numeric_columns[0] if numeric_columns else headers[1] if len(headers) > 1 else headers[0]
...
def output_node(state: CensusState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if charts_needed and census_data:
        ...
        for chart_spec in charts_needed:
            try:
                ...
                result = chart_tool._run(json.dumps({
    ...
    if tables_needed and census_data:  # Note: 'if', not 'elif'
        ...
        for table_spec in tables_needed:
            try:
                result = table_tool._run(json.dumps({
