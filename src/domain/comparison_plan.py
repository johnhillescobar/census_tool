...
DerivedMetric = Literal[
    "difference", "pct_difference", "rank", "percentile", "trend_gap"
]
...
class ComparisonPlan(BaseModel):
    ...
    missing_year_policy: MissingYearPolicy = Field(
        ..., description="The policy for handling missing years."
    )
    derived_metrics: list[DerivedMetric] = Field(
        ..., description="The derived metrics to calculate."
    )
    join_keys: list[JoinKey] = Field(..., description="The keys to join on.")
    requested_text: str | None = Field(
        default=None, description="The text that the user requested."
    )
    ...
    def validate_fields(self) -> "ComparisonPlan":
        ...
        if self.benchmark_geo_level is None and self.benchmark_geos:
            raise ValueError(
                "benchmark_geos must be empty when benchmark_geo_level is None"
            )
        if self.benchmark_geo_level is not None and not self.benchmark_geos:
            raise ValueError(
                "benchmark_geos must be non-empty when benchmark_geo_level is provided"
            )

        # subject_geo_level is required by type; keep this explicit defensive check
        if not self.subject_geos:
            raise ValueError(
                "subject_geos must be non-empty when subject_geo_level is provided"
            )
