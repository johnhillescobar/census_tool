...
def test_validate_variables_uses_chroma(monkeypatch):
    ...
    monkeypatch.setattr(variable_validator, "initialize_chroma_client", lambda: FakeClient(metadatas))
    monkeypatch.setattr(variable_validator, "_fetch_variables_json", lambda dataset, year: {})

    result = variable_validator.validate_variables(dataset="acs/acs5", year=2023, variables=["B01003_001E"])
...
def test_validate_variables_live_fallback(monkeypatch):
    monkeypatch.setattr(variable_validator, "initialize_chroma_client", lambda: FakeClient([]))
    ...
    monkeypatch.setattr(variable_validator, "_fetch_variables_json", lambda dataset, year: live_catalog)
...
def test_validate_variables_provides_alternatives(monkeypatch):
    ...
    result = variable_validator.validate_variables(dataset="acs/acs5", year=2023, variables=["B01001_009E"])
...
def test_list_variables_filters(monkeypatch):
    ...
    response = variable_validator.list_variables(dataset="acs/acs5", year=2023, table_code="B01003")
