...
def test_chart_request():
    ...
    try:
        ...
        if result.get('census_data'):
            ...
            if isinstance(result['census_data'], dict) and 'data' in result['census_data']:
                ...
                if isinstance(data_rows, list) and len(data_rows) > 0:
                    ...
                    print(f"Number of data rows: {len(data_rows) - 1 if len(data_rows) > 1 else 0}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(charts_dir):
        chart_files = [f for f in os.listdir(charts_dir) if f.endswith(('.png', '.html'))]
